...
allowedTools: [Agent, Read, Glob, Grep, TodoWrite, mcp__gh__gh_search]
---
...
Every artifact is identified by a stable `boule-id`. Before any create, the responsible agent (you, via the IPM and via subagents' `gh_search`) MUST search-before-create: `gh_search` for the `boule-id`. No match → create; match + same content-hash → no-op; match + different hash → update-in-place + audit comment. Re-running a run must CONVERGE on the same GitHub state, never spawn duplicates. Honor `--dry-run`: when dry-run is active, instruct the IPM to plan-and-print only, writing nothing.
...
# Autonomy boundaries
- You read GitHub (`mcp__gh__gh_search`) but you NEVER write — all writes go through the IPM.
