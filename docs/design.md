...
- Fully autonomous yet auditable through a single deterministic write chokepoint. Agents never call GitHub directly — every mutation passes the in-process `mcp__gh__*` tools wrapped by `canUseTool` + a `PreToolUse` hook, so idempotency (`boule-id` + `content-hash` block → no-op / update-in-place / create), `--dry-run`, dedupe (a unique per-artifact label + strongly-consistent List Issues — see the §State implementation note), blast-radius caps, and audit comments are implemented exactly once. Re-runs converge instead of accumulating duplicates — the precondition for trusting CI autonomy.
...
Artifacts optimize for **clarity, feasibility, traceability, and observability** — NOT cited evidence or success KPIs (both intentionally out of scope; designs ground themselves in real product/repo reality instead). A design is **`status:accepted`** only if: **Non-Goals is non-empty**; ≥1 job story in exact JTBD grammar (`When … I want to … so I can …` — a regex validator rejects role-based "As a…" here); it carries the canonical template sections including an **Approaches Considered** debate (2-3 genuinely distinct options with honest trade-offs → the most reasonable choice, with rationale), a **Feasibility** section grounded in the **target repository's** actual stack/architecture (as inventoried by the Repo Scout — Boule is product-agnostic and adapts to whatever repo it is run against; it never assumes its own architecture) without self-contradiction, and an **Observability** section making every Goal verifiable via concrete hooks that exist in that repo (logs / metrics / health checks / exit codes / events / tests); every job story is satisfiable by the Proposed Solution (traceability); and each Open Question carries a stable `OQ<n>` id with **no owner / @-mention** and is **autonomously resolved in-draft** (Decision + Rationale + Confidence in "Resolved Decisions"). Every artifact is decided through a bounded **approach debate**: the producer proposes the options, the Critic adjudicates (and may push for a different one) until APPROVE. Boule is fully autonomous — the Critic's APPROVE is the sole acceptance gate (no human review); nothing is deferred to a person. (`boule resolve <boule-id>` remains an *optional* human override; `boule:needs-human` is the dead-letter for genuine external blockers.) Body ≤ 65,536 chars (else split a large appendix into a sub-issue).
...
| `boule advance` | `--kind <kind>` `--budget` `--dry-run` | Autonomously push not-yet-accepted artifacts (`status:needs-review`/`draft`, board `Triage`/`In Design`/`In Review`) through the Critic gate to `status:accepted`/`Ready`, then into their next stage (accepted Design → Requirements). No human review — the Critic is the sole approver; unfixable items go to `boule:needs-human`. Runs after `triage` in the daily. |
...
| `boule resolve` | `[boule-id]` `--set OQ#=answer` `--from-comments` `--json` `--dry-run` | Answer an artifact's **Open Questions**. **Omit the boule-id** to discover open managed artifacts that still have Open Questions and pick one interactively (requires a TTY; not valid with `--json`, `--set`, or `--from-comments`). Interactive prompt by default; `--set` for scripting; `--from-comments` ingests answers **only from write/admin/maintain collaborators** (`getCollaboratorPermissionLevel`). Moves answered questions to a "Resolved Decisions" section, posts an audit comment, and refreshes the content-hash; `preserveResolutions` keeps decisions durable across agent re-runs. |
...
2. **Local artifact index** — `.boule/index.json`: a `bouleId -> { issueNumber, nodeId, contentHash }` map (also git-ignored). A fast local cache so dedupe doesn't require a GitHub round-trip every time. It is **advisory, not authoritative** — on a cache miss or a fresh clone, `upsertIssue` resolves against GitHub directly. This is the key resilience property: **deleting `.boule/` entirely must not cause duplicates**, because the true dedupe key lives on GitHub.

> **Implementation note (verified 2026-06, supersedes the search-based prose elsewhere in this doc).** Dedup is **NOT** done via GitHub issue *search* (`in:body "boule-id:…"`) — issue search is tokenized full-text and *eventually consistent*, so a just-created issue can be invisible to the index for seconds-to-minutes, letting duplicates slip through (and `:`/`-` tokenization breaks exact matching). The implemented mechanism: each artifact carries a **unique deterministic label** `boule-id-<sha256(bouleId)[:12]>`, and `upsertIssue` dedupes via the **REST List Issues** endpoint filtered by that exact label — strongly consistent, exact-match, no index lag. A **create-then-reconcile** pass (re-list by label after create; keep the lowest issue number, close the rest as duplicates) converges concurrent races. See `src/github/issues.ts`.
